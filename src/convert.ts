export function enter2space(str: string): string {
    return str.replace('\n', ' ').replace('    ', '')
}

export function aspirated2simple(consonant: string): string {
...
export function removeJosa(str: string): string {
    const josa = ['을', '를', '이', '가', '의']
    let newStr: string = str
    josa.forEach((i) => newStr = newStr.replace(i, ''))
    return newStr
}
